...
// 클래스의 용도 : 현재 사용하는 시점의 요청에 있는 쿠키값들을 저장해서 기능을 제공하는 클래스
// 쿠키 생성 메소드, 쿠키 제공 메소드, 쿠키값 제공 메소드, 쿠키 존재유무 확인 메소드
public class CookieBox {
	
	// 쿠키 값을 저장한다 : 간단한 코드로 빠르게 참고하고자 한다
	// -> HashMap<String, Cookie>에 저장
	private Map<String, Cookie> cookieMap = new HashMap<String, Cookie>();
	
	// 생성자를 통해 값을 초기화 : Map 에 Cookie 객체들을 저장
	...
	// 쿠키 생성 메소드 : 빠른 처리를 위해 인스턴스 생성 없이 사용 가능하도록 한다. -> static
	// 쿠키 생성시에 필요한 데이터 : 쿠키 이름, 쿠키값, 유효시간, 경로, 도메인
	// 오버로딩을 통해 같은 이름의 메소드를 정의
	...
	// 쿠키 존재 유무 확인 : 존재하면 true, 아니면 false
